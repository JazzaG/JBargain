...
public class ElementCommentIterator implements Iterator<List<Comment>> {
    ...
    public ElementCommentIterator(ScraperUser user, String id, Element element) {
        this.user = user;
        this.id = id;
    ...
    public List<Comment> next() {
        ...
        if (element != null) {
            ...
            for (Element item : element.select("ul.comment li")) {
                ...
                // Add comment
                comments.add(new ElementCommentAdapter(item, level));
            }
        }
